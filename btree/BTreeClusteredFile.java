...
/**
 * BTreeClusteredFile.java
 * This is the main definition of class BTreeClusteredFile, which derives from
 * abstract base class ClusteredIndexFile.
 * It provides an insert/delete interface.
 */
public class BTreeClusteredFile extends ClusteredIndexFile
     ...
     */
    public void insert(KeyClass key, PageId pageId)
        ...
        if (trace != null) {
            trace.writeBytes("INSERT " + pageId.pid + " " + key + lineSep);
        ...
        if (headerPage.get_rootId().pid == INVALID_PAGE) {
            ...
            newRootPage = new BTClusteredLeafPage(headerPage.get_keyType());
            ...
            newRootPage.insertRecord(key, pageId);
        ...
        newRootEntry = _insert(key, pageId, headerPage.get_rootId());
    ...
    private KeyDataEntry _insert(KeyClass key, PageId pageId,
        ...
        if (currentPage.getType() == NodeType.INDEX) {
            ...
            upEntry = _insert(key, pageId, nextPageId);
        ...
        } else if (currentPage.getType() == NodeType.CLUSTERED_LEAF) {
            BTClusteredLeafPage currentLeafPage =
                    new BTClusteredLeafPage(page, headerPage.get_keyType());
            ...
            if (currentLeafPage.available_space() >=
                    BT.getKeyDataLength(key, NodeType.CLUSTERED_LEAF)) {
                // no split has occurred

                currentLeafPage.insertRecord(key, pageId);
            ...
            newLeafPage = new BTClusteredLeafPage(headerPage.get_keyType());
            ...
            if (rightPageId.pid != INVALID_PAGE) {
                ...
                rightPage = new BTClusteredLeafPage(rightPageId, headerPage.get_keyType());
            ...
            if (BT.keyCompare(key, undoEntry.key) >= 0) {
                ...
                newLeafPage.insertRecord(key, pageId);
     ...
     *@return return a BTClusteredLeafPage instance which is pinned.
    ...
    BTClusteredLeafPage findRunStart(KeyClass lo_key,
        ...
        pageLeaf = new BTClusteredLeafPage(page, headerPage.get_keyType());
        ...
        while (curEntry == null) {
            ...
            pageno = nextpageno;
            pageLeaf = new BTClusteredLeafPage(pinPage(pageno), headerPage.get_keyType());
            curEntry = pageLeaf.getFirst(startrid);
        }
     ...
     * BTClusteredLeafPage::delUserRid.
     */

    private boolean NaiveDelete(KeyClass key, PageId pageId)
        ...
        if (trace != null) {
            trace.writeBytes("DELETE " + pageId.pid +" "
                    + key + lineSep);
            trace.writeBytes("DO" + lineSep);
            trace.writeBytes("SEARCH" + lineSep);
            trace.flush();
        ...
        while (true) {
            ...
            leafPage = new BTClusteredLeafPage(pinPage(nextpage), headerPage.get_keyType());
    ...
    private boolean FullDelete(KeyClass key, PageId pageId)
        ...
        try {
            ...
            _Delete(key, pageId, headerPage.get_rootId(), null);
    ...
    private KeyClass _Delete(KeyClass key,
        ...
        if (sortPage.getType() == NodeType.CLUSTERED_LEAF) {
            RID curRid = new RID();  // iterator
            KeyDataEntry tmpEntry;
            KeyClass curkey;
            RID dummyRid;
            PageId nextpage;
            BTClusteredLeafPage leafPage;
            leafPage = new BTClusteredLeafPage(page, headerPage.get_keyType());
            ...
            while ((tmpEntry != null) && (BT.keyCompare(key, tmpEntry.key) >= 0)) {
                // WriteUpdateLog is done in the BTClusteredLeafPage level - to log the
                // deletion of the rid.
        ...
        if (sortPage.getType() == NodeType.INDEX) {
            ...
            KeyClass oldChildKey = _Delete(key, pageId, childPageId, currentPageId);
    ...
    void trace_children(PageId id)
        ...
        if (trace != null) {
            ...
            if (sortedPage.getType() == NodeType.INDEX) {
                ...
                BTClusteredLeafPage leafPage = new BTClusteredLeafPage(sortedPage, headerPage.get_keyType());
