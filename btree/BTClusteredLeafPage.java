...
/**
 * A BTClusteredLeafPage is a leaf page on a B+ tree.  It holds abstract
...
public class BTClusteredLeafPage extends BTSortedPage {
    ...
    /**
     * pin the page with pageno, and get the corresponding BTClusteredLeafPage,
     * also it sets the type to be NodeType.CLUSTERED_LEAF.
     *
     * @param pageno  Input parameter. To specify which page number the
     *                BTClusteredLeafPage will correspond to.
     ...
     */
    public BTClusteredLeafPage(PageId pageno, int keyType)
            throws IOException,
            ConstructPageException {
        super(pageno, keyType);
        setType(NodeType.CLUSTERED_LEAF);
    }

    /**
     * associate the BTClusteredLeafPage instance with the Page instance,
     * also it sets the type to be NodeType.CLUSTERED_LEAF.
     *
     ...
     * @throws ConstructPageException BTLeafPage constructor error
     */
    public BTClusteredLeafPage(Page page, int keyType)
            throws IOException,
            ConstructPageException {
        super(page, keyType);
        setType(NodeType.CLUSTERED_LEAF);
    ...
    /**
     * new a page, associate the BTClusteredLeafPage instance with the Page instance,
     * also it sets the type to be NodeType.CLUSTERED_LEAF.
     *
     * @param keyType either AttrType.attrInteger or AttrType.attrString.
     *                Input parameter.
     * @throws IOException            error from the lower layer
     * @throws ConstructPageException BTClusteredLeafPage constructor error
     */
    public BTClusteredLeafPage(int keyType)
            throws IOException,
            ConstructPageException {
        super(keyType);
        setType(NodeType.CLUSTERED_LEAF);
     ...
     * @param dataPid - the pageid of the data record. This is
     ...
     */
    public RID insertRecord(KeyClass key, PageId dataPid)
            throws LeafInsertRecException {
        KeyDataEntry entry;

        try {
            entry = new KeyDataEntry(key, dataPid.pid);
    ...
    public KeyDataEntry getFirst(RID rid)
        ...
        try {
            ...
            entry = BT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0),
                    keyType, NodeType.CLUSTERED_LEAF);
     ...
     * iterator interface to the records on a BTClusteredLeafPage.
     *
            ...
            if (direction == -1) { // 'this' is the left sibling of leafPage
                if ((getSlotLength(getSlotCnt() - 1) + available_space() + 8 /*  2*sizeof(slot) */) >
                    ...
                    lastEntry = BT.getEntryFromBytes(getpage(), getSlotOffset(getSlotCnt() - 1)
                            , getSlotLength(getSlotCnt() - 1), keyType, NodeType.CLUSTERED_LEAF);
